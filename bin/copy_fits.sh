...
for hnl_mass in "${strings[@]}"; do
    ...
    sigma_file="${subdir}/sigma-nu-N-${current}-GRV98lo_patched_central"
    sigmabar_file="${subdir}/sigma-nubar-N-${current}-GRV98lo_patched_central"
    dsigma_file="${subdir}/dsdxdy-nu-N-${current}-GRV98lo_patched_central"
    dsigmabar_file="${subdir}/dsdxdy-nubar-N-${current}-GRV98lo_patched_central"
    mkdir "${outdir}/${subdir}"
